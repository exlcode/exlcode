...
import { IWindowsService, Menu, MenuItem } from "exlcode/fakeElectron";
import * as Electron from "exlcode/fakeElectron";
...
import { AboutEXLcodeAction } from "exlcode/exlcodeActions";
...
export class VSCodeMenu {
	...
	private install(): void {
		...
		if (platform.isMacintosh) {
			...
			macApplicationMenuItem = new MenuItem({
				label: this.envService.product.nameShort,
	...
	*/
	...
	private setMacApplicationMenu(macApplicationMenu: Electron.Menu): void {
		...
		let about = this.createMenuItem(
			nls.localize(
				"mAbout",
				"About {0}",
				this.envService.product.nameLong
			),
			AboutEXLcodeAction.ID
		);
	...
	private setFileMenu(fileMenu: Electron.Menu): void {
		...
		*/
		...
		let saveFile = this.createMenuItem(
			nls.localize(
		...
		let saveFileAs = this.createMenuItem(
			nls.localize(
		...
		let saveAllFiles = this.createMenuItem(
			nls.localize(
		...
		let revertFile = this.createMenuItem(
			nls.localize(
	...
	private getPreferencesMenu(): Electron.MenuItem {
		let userSettings = this.createMenuItem(
			nls.localize(
		...
		let workspaceSettings = this.createMenuItem(
			nls.localize(
				{
		...
		let kebindingSettings = this.createMenuItem(
			nls.localize(
		...
		let snippetsSettings = this.createMenuItem(
			nls.localize(
		...
		let colorThemeSelection = this.createMenuItem(
			nls.localize(
				{
		...
		let iconThemeSelection = this.createMenuItem(
			nls.localize(
				{
	...
	*/
	...
	private setEditMenu(winLinuxEditMenu: Electron.Menu): void {
		...
		*/
		...
		let replace = this.createMenuItem(
			nls.localize(
		...
		let findInFiles = this.createMenuItem(
			nls.localize(
		...
		let replaceInFiles = this.createMenuItem(
			nls.localize(
	...
	private setViewMenu(viewMenu: Electron.Menu): void {
		let explorer = this.createMenuItem(
			nls.localize(
		...
		let search = this.createMenuItem(
			nls.localize(
		...
		let extensions = this.createMenuItem(
			nls.localize(
		...
		let output = this.createMenuItem(
			nls.localize(
		...
		let debugConsole = this.createMenuItem(
			nls.localize(
				{
		...
		let integratedTerminal = this.createMenuItem(
			nls.localize(
				{
		...
		let problems = this.createMenuItem(
			nls.localize(
		...
		let commands = this.createMenuItem(
			nls.localize(
		...
		let toggleMenuBar = this.createMenuItem(
			nls.localize(
		...
		let splitEditor = this.createMenuItem(
			nls.localize(
		...
		let toggleSidebar = this.createMenuItem(
			nls.localize(
		...
		let moveSidebar = this.createMenuItem(
			nls.localize(
		...
		let togglePanel = this.createMenuItem(
			nls.localize(
		...
		let toggleStatusbar = this.createMenuItem(
			nls.localize(
				{
		...
		let zoomIn = this.createMenuItem(
			nls.localize(
		...
		let zoomOut = this.createMenuItem(
			nls.localize(
		...
		let resetZoom = this.createMenuItem(
			nls.localize(
	...
	private setGotoMenu(gotoMenu: Electron.Menu): void {
		...
		let forward = this.createMenuItem(
			nls.localize(
		...
		let nextEditor = this.createMenuItem(
			nls.localize(
		...
		let previousEditor = this.createMenuItem(
			nls.localize(
		...
		let nextEditorInGroup = this.createMenuItem(
			nls.localize(
				{
		...
		let previousEditorInGroup = this.createMenuItem(
			nls.localize(
				{
		...
		let focusFirstGroup = this.createMenuItem(
			nls.localize(
				{
		...
		let focusSecondGroup = this.createMenuItem(
			nls.localize(
				{
		...
		let focusThirdGroup = this.createMenuItem(
			nls.localize(
				{
		...
		let nextGroup = this.createMenuItem(
			nls.localize(
		...
		let previousGroup = this.createMenuItem(
			nls.localize(
		...
		let gotoFile = this.createMenuItem(
			nls.localize(
		...
		let gotoDefinition = this.createMenuItem(
			nls.localize(
		...
		let gotoLine = this.createMenuItem(
			nls.localize(
		...
		let gotoRepo = this.createMenuItem(
			nls.localize("miGotoRepository", "Go to &&Repository..."),
			"workbench.action.exlcode.chooseRepository"
		);
		let gotoRef = this.createMenuItem(
			nls.localize("miGotoReference", "Go to &&Branch or Tag..."),
			"workbench.action.exlcode.chooseReference"
		);
	...
	private setMacWindowMenu(macWindowMenu: Electron.Menu): void {
		let minimize = new MenuItem({
			label: nls.localize("mMinimize", "Minimize"),
			role: "minimize",
			accelerator: "Command+M",
			enabled: this.windowsService.getWindowCount() > 0
		});
		let close = new MenuItem({
			label: nls.localize("mClose", "Close"),
			role: "close",
			accelerator: "Command+W",
			enabled: this.windowsService.getWindowCount() > 0
		});
		let bringAllToFront = new MenuItem({
			label: nls.localize("mBringToFront", "Bring All to Front"),
			role: "front",
			enabled: this.windowsService.getWindowCount() > 0
		});

		[minimize, close, __separator__(), bringAllToFront].forEach(item =>
			macWindowMenu.append(item)
		);
	}
	...
	*/
	...
	private setHelpMenu(helpMenu: Electron.Menu): void {
		...
		*/
		...
		arrays
			.coalesce([
				this.envService.product.documentationUrl
					? new MenuItem({
							label: mnemonicLabel(
								nls.localize(
									{
				...
				this.envService.product.releaseNotesUrl
					? __separator__()
					: null,
				this.envService.product.twitterUrl
					? new MenuItem({
							label: mnemonicLabel(
								nls.localize(
									{
					...
					: null,
				this.envService.product.sendFeedbackUrl
					? new MenuItem({
							label: mnemonicLabel(
								nls.localize(
									{
					...
					: null,
				this.envService.product.requestFeatureUrl
					? new MenuItem({
							label: mnemonicLabel(
								nls.localize(
									{
					...
					: null,
				this.envService.product.reportIssueUrl
					? new MenuItem({
							label: mnemonicLabel(
								nls.localize(
									{
				...
				this.envService.product.reportIssueUrl
					? __separator__()
					: null,
				this.envService.product.licenseUrl
					? new MenuItem({
							label: mnemonicLabel(
								nls.localize(
									{
							...
							click: () => {
								if (platform.language) {
									let queryArgChar =
										this.envService.product.licenseUrl.indexOf(
											"?"
										) > 0
											? "&"
											: "?";
	...
	*/
	...
	private getAccelerator(actionId: string): string {
		if (actionId) {
			let resolvedKeybinding = this.mapResolvedKeybindingToActionId[
				actionId
			];
