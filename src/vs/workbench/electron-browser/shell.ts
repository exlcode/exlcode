...
} from "vs/workbench/common/storage";
import {
	ITelemetryService,
...
import { NavbarPart } from "exlcode/navbarPart";
import { INavbarService, NavbarAlignment } from "exlcode/navbarService";
import { UserNavbarItem } from "exlcode/userNavbarItem";
import { MenusNavbarItem } from "exlcode/menusNavbarItem";
import { IExlcodeService } from "exlcode/exlcodeService";
import { WelcomePart } from "exlcode/welcomePart";
import {
	ChooseRepositoryAction,
	ChooseReferenceAction,
	ChooseContextAction,
	AboutEXLcodeAction
} from "exlcode/exlcodeActions";
import {
...
var updateEnablementPrev =
	fileActions.BaseFileAction.prototype._updateEnablement;
...
goToDeclaration.DefinitionAction.prototype.run = function() {
	showTip(
		"definitionActionTip",
		"Note: Go To and Peek Definition only work within opened files in EXLcode."
	);
...
referenceSearch.ReferenceAction.prototype.run = function() {
	showTip(
		"referenceSearchTip",
		"Note: Find All References only works within opened files in EXLcode."
	);
...
const ShowDocumentationAction = new Action(
	...
	() => {
		window.open(
			"https://exlcode.com/documentation?welcome=1",
	...
	if (!getKeyValue("welcomeTip")) {
		...
		g_messageService.show(Severity.Info, {
			message: nls.localize(
				"readDocumentation",
				"Welcome to EXLcode! Would you like to read the documentation?"
			),
			actions: [CloseAction, ShowDocumentationAction]
		});
	}
}
	...
	switch (hack) {
		...
		case BrowserHack.MESSAGE_BOX_TEXT:
			if (isIE || isEdge) {
				...
				(<any>style.sheet).insertRule(
					".message-left-side.message-overflow-ellipsis { overflow: visible !important; }",
					0
				);
			}
			break;
		...
		case BrowserHack.TAB_LABEL:
			if (isIE || isEdge) {
				...
				(<any>style.sheet).insertRule(
					".monaco-workbench > .part.editor > .content > .one-editor-silo > .container > .title .tabs-container > .tab { width: auto !important; }",
					0
				);
				...
				if (isIE) {
					// min-width: fit-content doesn't work on IE or Edge
					(<any>style.sheet).insertRule(
						".monaco-workbench > .part.editor > .content > .one-editor-silo > .container > .title .tabs-container > .tab { display: inline-block !important; }",
						0
					);
					...
					(<any>style.sheet).insertRule(
						".tab-label { margin-top: 6% !important; display: inline-block !important; }",
						0
					);
					(<any>style.sheet).insertRule(
						".tab-close { margin-top: 8% !important; display: inline-block !important; }",
						0
					);
				}
			}
...
export class WorkbenchShell {
	...
	constructor(
		...
		this.exlcodeService = services.exlcodeService;

		this.toUnbind = [];
		this.previousErrorTime = 0;

		// Set read only state for file actions
		fileActionsReadOnly = this.exlcodeService.isTag;
	}
	...
	private createContents(parent: Builder): Builder {
		...
		ensureStaticPlatformServices({
			...
			markerService: <IMarkerService>(
				serviceCollection.get(IMarkerService)
			),
			messageService: <IMessageService>(
				serviceCollection.get(IMessageService)
			),
			commandService: <ICommandService>(
				serviceCollection.get(ICommandService)
			),
			compatWorkerService: <ICompatWorkerService>(
				serviceCollection.get(ICompatWorkerService)
			),
			storageService: <IStorageService>(
				serviceCollection.get(IStorageService)
			),
			telemetryService: <ITelemetryService>(
				serviceCollection.get(ITelemetryService)
			),
			contextService: <IWorkspaceContextService>(
				serviceCollection.get(IWorkspaceContextService)
			),
			eventService: <IEventService>serviceCollection.get(IEventService),
			extensionService: <IExtensionService>(
				serviceCollection.get(IExtensionService)
			),
			configurationService: <IConfigurationService>(
				serviceCollection.get(IConfigurationService)
			),
			codeEditorService: <ICodeEditorService>(
				serviceCollection.get(ICodeEditorService)
			),
			editorWorkerService: <IEditorWorkerService>(
				serviceCollection.get(IEditorWorkerService)
			),
		...
		this.workbench.startup({
			onWorkbenchStarted: customKeybindingsCount => {
				...
				if (
					...
					let builtInActionDescriptor = (<IWorkbenchActionRegistry>(
						Registry.as(ActionExtensions.WorkbenchActions)
					)).getWorkbenchAction(id);
					if (builtInActionDescriptor) {
						let action: IAction = instantiationService.createInstance(
			...
			onServicesCreated: () => {
				...
				this.navbarPart = new NavbarPart(
					Identifiers.NAVBAR_PART,
					instantiationService
				);
				...
				// Tell exlcodeSearch about the editor service. Search needs to use IModels
				...
				// of matching uris.
				let editorService = <IWorkbenchEditorService>(
					serviceCollection.get(IWorkbenchEditorService)
				);
				let searchService = <ISearchService>(
					serviceCollection.get(ISearchService)
				);
				(<any>searchService).exlcodeSearch.setEditorService(
					editorService
				);
			}
		});
	...
	private fillNavbar(instantiationService: InstantiationService): void {
		...
		if (!this.isWelcomeMode()) {
			...
			menu.ready();
			let menusItem = instantiationService.createInstance(
				MenusNavbarItem
			);
		...
		this.navbarPart.addEntry(
			{
				text:
					"$(bell)" +
					(this.isWelcomeMode() ? " EXLcode" : "") +
					((<IWindowConfiguration>(<any>this.environmentService))
						.readOnly
						? " (read only)"
						: ""),
				tooltip: AboutEXLcodeAction.LABEL,
				command: AboutEXLcodeAction.ID
			},
		...
		if (this.exlcodeService.isAuthenticated()) {
			this.navbarPart.addEntry(
				{
					text: this.exlcodeService.repoName
						? this.exlcodeService.repoName
		...
		} else if (this.exlcodeService.repoName) {
			this.navbarPart.addEntry(
				{
					text: this.exlcodeService.repoName
				},
		...
		if (this.exlcodeService.repoName) {
			this.navbarPart.addEntry(
				{
					text: this.exlcodeService.ref,
			...
			/* TODO show navbar entry to context choosing once it is ready for public use
			this.navbarPart.addEntry(
				{
					text: this.exlcodeService.ref,
					tooltip: ChooseContextAction.LABEL,
					command: ChooseContextAction.ID
				},
				NavbarAlignment.RIGHT,
				450
			);
	...
	private createWelcomePart(): void {
		this.welcomePart = new WelcomePart(
			Identifiers.WELCOME_PART,
			this.exlcodeService
		);
	...
	private isWelcomeMode(): boolean {
		...
		// 	!this.exlcodeService.repoName
		// );
		return !this.exlcodeService.repoName;
	}
	...
	private onWorkbenchStarted(customKeybindingsCount: number): void {
		...
		const workspaceStats: WorkspaceStats = <WorkspaceStats>(
			this.workbench
		...
		*/
		...
		require([
			...
			let global: any = self;
			let MonacoEditorSchemas: { [url: string]: IJSONSchema } =
				global.MonacoEditorSchemas;
			let schemas = [];
			for (var uri in MonacoEditorSchemas) {
				...
				schemas.push({
					uri: uri,
					fileMatch: [pattern],
					schema: MonacoEditorSchemas[uri]
				});
			}
	...
	private initServiceCollection(): [InstantiationService, ServiceCollection] {
		...
		*/
		...
		// EXLcode: We've unified the env/ironmentService into a single data type and instance. Hopefully one will go away.
		serviceCollection.set(IEnvService, <IEnvService>(
			(<any>this.environmentService)
		));
		...
		const disableWorkspaceStorage =
			...
			(!this.workspace &&
				!this.environmentService.extensionDevelopmentPath); // without workspace or in any extension test, we use inMemory storage unless we develop an extension where we want to preserve state
		...
		if (
			...
			!this.environmentService
				.extensionDevelopmentPath /* && !!product.enableTelemetry */
		) {
			/* TODO:
		...
		serviceCollection.set(IExlcodeService, this.exlcodeService);
		...
		const fileService = disposables.add(
			instantiationService.createInstance(FileService)
		);
		fileService.updateOptions({
			...
			gistRegEx: (<IWindowConfiguration>(<any>this.environmentService))
				.gistRegEx
		});
		serviceCollection.set(IFileService, fileService);

		this.toUnbind.push(
			NullLifecycleService.onShutdown(() => disposables.dispose())
		);
		...
		*/
		...
		const extensionService = instantiationService.createInstance(
		...
		serviceCollection.set(
			ICommandService,
			new CommandService(instantiationService, extensionService)
		);
		...
		*/
		...
		const searchService = instantiationService.createInstance(
			SearchService
		);
		serviceCollection.set(ISearchService, searchService);

		const codeEditorService = instantiationService.createInstance(
	...
	public open(): void {
		...
		// If the user isn't authenticated and has not selected a repo show a special welcome to help them get started.
		if (
			!this.exlcodeService.isAuthenticated() &&
			!this.exlcodeService.repoName
		) {
		...
		if (timers) {
			...
			if (timers.vscodeStart) {
				events.push({
					...
					name: "VSCode Startup",
					description:
						"Time it takes to create a window and startup VSCode"
				});
			}
			...
			events.push({
				...
				description:
					"Time it takes for the DOM to emit DOMContentLoaded event"
			});

			// Write to Timer
			timer
				.getTimeKeeper()
				.setInitialCollectedEvents(events, timers.start);
		}
	}
	...
	public layout(): void {
		...
		let navbarHeight =
			parseInt(navbarStyle.getPropertyValue("height"), 10) || 18;
		this.navbarPart.getContainer().position(0);
		this.contentsContainer.position(navbarHeight);
		this.contentsContainer.size(
			contentsSize.width,
			contentsSize.height - navbarHeight
		);
