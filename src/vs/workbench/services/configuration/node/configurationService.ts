...
// DESKTOP: import paths = require('vs/base/common/paths');
import paths = require("vs/base/common/paths");
// DESKTOP: import extfs = require('vs/base/node/extfs');
...
// DESKTOP: import {readFile} from 'vs/base/node/pfs';
import errors = require("vs/base/common/errors");
import {
	IConfigFile,
	consolidate,
	newConfigFile
} from "vs/workbench/services/configuration/common/model";
import {
...
import { Exlcode, Repository, Error as ExlcodeError } from "exlcode";
...
var g_cache: IExlcodeTreeCache;
var g_exlcodeService: IExlcodeService;
...
export class WorkspaceConfigurationService
	...
	private workspaceFilePathToConfiguration: {
		[relativeWorkspacePath: string]: TPromise<IConfigFile>;
	};
	...
	constructor(
		...
		this.baseConfigurationService = new BaseConfigurationService(
			environmentService,
			eventService,
			exlcodeService
		);
		...
		g_cache = this.cache;
		g_exlcodeService = this.exlcodeService;
	...
	private registerListeners(): void {
		this.toDispose.push(
			this.eventService.addListener2(FileEventType.FILE_CHANGES, events =>
				this.handleWorkspaceFileEvents(events)
			)
		);
		this.toDispose.push(
			this.baseConfigurationService.onDidUpdateConfiguration(() =>
				this.onBaseConfigurationChanged()
			)
		);
		this.toDispose.push(
			this.eventService.addListener2("settingsFileChanged", events => {
				this.reloadConfiguration().then(() =>
	...
	public lookup<C>(key: string): IWorkspaceConfigurationValue<C> {
		...
		return {
			...
			workspace: getConfigurationValue<C>(
	...
	public reloadConfiguration(section?: string): TPromise<any> {
		...
		return this.baseConfigurationService
			.reloadConfiguration()
			.then(() => this.doLoadConfiguration(section));
	}

	private doLoadConfiguration(section?: string): TPromise<any> {
		// Load workspace locals
		return this.loadWorkspaceConfigFiles()
			.then(workspaceConfigFiles => {
				// Consolidate (support *.json files in the workspace settings folder)
				const workspaceConfig = consolidate(workspaceConfigFiles)
					.contents;
				...
				return objects.mixin(
					objects.clone(
						this.baseConfigurationService.getConfiguration()
					), // target: global/default values (do NOT modify)
					this.cachedWorkspaceConfig, // source: workspace configured values
					true // overwrite
				);
			})
	...
	public hasWorkspaceConfiguration(): boolean {
		return !!this.workspaceFilePathToConfiguration[
			`${this.workspaceSettingsRootFolder}/${CONFIG_DEFAULT_NAME}.json`
		];
	}
	...
	private loadWorkspaceConfigFiles(): TPromise<{
					...
					stat => {
						...
						return resolveContents(
							stat.children
								.filter(
									stat =>
										paths.extname(stat.resource.fsPath) ===
										".json"
								)
					...
					contents.forEach(
						content =>
							(this.workspaceFilePathToConfiguration[
								this.contextService.toWorkspaceRelativePath(
									content.resource
								)
							] = TPromise.as(newConfigFile(content.value)))
					);
	...
	private handleWorkspaceFileEvents(event: FileChangesEvent): void {
		...
		for (let i = 0, len = events.length; i < len; i++) {
			...
			if (
				paths.extname(workspacePath) !== ".json" ||
			...
			switch (events[i].type) {
				...
				case FileChangeType.ADDED:
					this.workspaceFilePathToConfiguration[
						workspacePath
					] = resolveContent(events[i].resource).then(
						content => newConfigFile(content.value),
						errors.onUnexpectedError
					);
					affectedByChanges = true;
			}
		...
		if (
			affectedByChanges &&
			!this.reloadConfigurationScheduler.isScheduled()
		) {
...
function resolveContent(resource: uri): TPromise<IContent> {
	return new TPromise<IContent>((c, e) => {
		g_cache.stat(resource.fsPath, (error: any, stat?: IExlcodeTreeStat) => {
			if (error) {
				e("Error loading " + resource.fsPath);
				return;
			}
			g_exlcodeService.repo.getBlobRaw(
				stat.sha,
				(err: ExlcodeError, content: string | boolean) => {
					if (!err) {
						...
						c({
							resource: resource,
							value: content == true ? "" : <string>content
						});
					} else {
						e("Error loading " + resource.fsPath);
					}
...
function resolveStat(resource: uri): TPromise<IStat> {
	return new TPromise<IStat>((c, e) => {
		g_cache.readdir(resource.fsPath, (error, children) => {
			if (error) {
				...
				c({
					...
					children: children.map(child => {
						return {
							resource: uri.file(
								paths.join(resource.fsPath, child)
							)
						};
					})
				});
