...
import "exlcode/lib/css.escape"; // Polyfill for CSS.escape() which is Chrome/FF on at the moment.
...
import {
	...
	IThemeSetting,
	IThemeDocument
} from "vs/workbench/services/themes/common/themeService";
import {
	TokenStylesContribution,
	EditorStylesContribution,
	SearchViewStylesContribution
} from "vs/workbench/services/themes/electron-browser/stylesContributions";
...
// TODO: import * as plist from 'fast-plist';
		...
		defaultSnippets: [
			{
				body: {
					id: "{{id}}",
					label: "{{label}}",
					path: "./fileicons/{{id}}-icon-theme.json"
				}
			}
		],
		properties: {
...
export class ThemeService implements IThemeService {
	...
	constructor(
		...
		for (let theme of themesInitialize) {
			this.onIconThemes("", theme.extensionId, theme.iconThemes, null);
		}
		/* DESKTOP:
	...
	public initialize(container: HTMLElement): TPromise<void> {
		...
		let themeId = this.storageService.get(
			COLOR_THEME_PREF,
			StorageScope.GLOBAL,
			null
		);
		...
		let iconThemeId = this.storageService.get(
			ICON_THEME_PREF,
			StorageScope.GLOBAL,
			"vscode.theme-defaults-vs-seti"
		);
		return Promise.join([
	...
	private applyThemeCSS(
		themeId: string,
		defaultId: string,
		onApply: (theme: IInternalThemeData) => void
	): TPromise<boolean> {
		let rootPath =
			(<IWindowConfiguration>(<any>this.environmentService)).rootPath +
			"/";
	...
	private onThemes(
		...
		themes.forEach(theme => {
			...
			let themeSelector = toCSSSelector(
				extensionId + "-" + Paths.normalize(theme.path)
			);
	...
	private sendTelemetry(themeData: IInternalThemeData) {
		if (!this.themeExtensionsActivated[themeData.extensionId]) {
			let description = ExtensionsRegistry.getExtensionDescription(
				themeData.extensionId
			);
			if (description) {
	...
	private _updateIconTheme(
		...
		return this.getFileIconThemes().then(allIconSets => {
			...
			return _applyIconTheme(iconSetData, rootPath, onApply);
		});
	}
}

function _applyIconTheme(
	data: IInternalThemeData,
	rootPath: string,
	onApply: (theme: IInternalThemeData) => void
): TPromise<boolean> {
	if (!data) {
	...
	return _loadIconThemeDocument(data.path, rootPath).then(
		iconThemeDocument => {
			let styleSheetContent = _processIconThemeDocument(
				data.id,
				rootPath + data.path,
				iconThemeDocument
			);
...
function _loadIconThemeDocument(
	...
	// DESKTOP:	return pfs.readFile(fileSetPath).then(content => {
	return xhr({ type: "GET", url: rootPath + fileSetPath }).then(
		(xhr: XMLHttpRequest) => {
			let content = xhr.responseText;
			let errors: Json.ParseError[] = [];
			let contentValue = <IThemeDocument>(
				Json.parse(content.toString(), errors)
			);
			if (errors.length > 0) {
				return TPromise.wrapError(
					new Error(
						nls.localize(
							...
							errors
								.map(e => Json.getParseErrorMessage(e.error))
								.join(", ")
						)
					)
				);
		...
		if (associations) {
			...
			if (fileExtensions) {
				for (let fileExtension in fileExtensions) {
					addSelector(
						...
						)}-ext-file-icon.file-icon::before`,
			...
			if (fileNames) {
				for (let fileName in fileNames) {
					...
					if (segments[0]) {
						selectors.push(
							`.${escapeCSS(segments[0])}-name-file-icon`
						);
					}
	...
	if (Array.isArray(fonts)) {
		fonts.forEach(font => {
			let src = font.src
				...
				.join(", ");
			cssRules.push(
				`@font-face { src: ${src}; font-family: '${
					font.id
				}'; font-weigth: ${font.weight}; font-style: ${font.style}; }`
			);
		});
		cssRules.push(
			`.show-file-icons .file-icon::before, .show-file-icons .folder-icon::before { font-family: '${
				fonts[0].id
...
function applyTheme(
	...
	return _loadThemeDocument(theme.path, rootPath).then(
		themeDocument => {
			let styleSheetContent = _processThemeObject(
				theme.id,
				themeDocument
			);
...
function _loadThemeDocument(
	themePath: string,
	rootPath: string
): TPromise<IThemeDocument> {
	// return pfs.readFile(themePath).then(content => {
	return xhr({ type: "GET", url: rootPath + themePath }).then(
		(xhr: XMLHttpRequest) => {
			let content = xhr.responseText;
			if (Paths.extname(themePath) === ".json") {
					...
					return TPromise.wrapError(
						new Error(
							nls.localize(
								...
								errors
									.map(e =>
										Json.getParseErrorMessage(e.error)
									)
									.join(", ")
							)
						)
				...
				if (contentValue.include) {
					return _loadThemeDocument(
						Paths.join(
							Paths.dirname(themePath),
							contentValue.include
						),
						rootPath
					).then(includedValue => {
						contentValue.settings = includedValue.settings.concat(
...
function _processThemeObject(
	themeId: string,
	themeDocument: IThemeDocument
): string {
	let cssRules: string[] = [];
	...
	if (Array.isArray(themeSettings)) {
		cssRules = cssRules.concat(
			new TokenStylesContribution().contributeStyles(
				themeId,
				themeDocument
			)
		);
		cssRules = cssRules.concat(
			new EditorStylesContribution().contributeStyles(
				themeId,
				themeDocument
			)
		);
		cssRules = cssRules.concat(
			new SearchViewStylesContribution().contributeStyles(
				themeId,
				themeDocument
			)
		);
...
const themesInitialize = [
	{
		...
		iconThemes: [
			...
			},
			{
				...
				path: "./icons/vs_minimal_icons.json"
			}
		]
	}
];
...
let schemaRegistry = <IJSONContributionRegistry>(
	Registry.as(JSONExtensions.JSONContribution)
);
