...
import { Engine as ExlcodeFileSearchEngine } from "vs/workbench/services/search/node/fileSearch";
...
export class SearchService implements ISearchService {
	...
	private exlcodeSearch: ExlcodeSearch;
	...
	constructor(
		...
		this.exlcodeSearch = new ExlcodeSearch(this.exlcodeService);
	}
	...
	public search(
		...
		return new PPromise<ISearchComplete, ISearchProgressItem>(
			(onComplete, onError, onProgress) => {
				...
				rawSearchQuery = this.exlcodeSearch.search(query).then(
					// on Complete
					complete => {
	...
	private getLocalResults(
		query: ISearchQuery
	): { [resourcePath: string]: IFileMatch } {
		let localResults: {
			[resourcePath: string]: IFileMatch;
		} = Object.create(null);
	...
	private matches(
		...
		if (includePattern) {
			...
			if (
				!glob.match(
					includePattern,
					workspaceRelativePath || resource.fsPath
				)
			) {
				return false;
			}
...
class ExlcodeSearch {
	...
	constructor(private exlcodeService: IExlcodeService) {
	...
	private textSearch(
		...
		return new PPromise<ISearchComplete, ISearchProgressItem>((c, e, p) => {
			...
			if (!this.exlcodeService.isDefaultBranch()) {
				let br = this.exlcodeService.getDefaultBranch();
				e(
					"Exlcode only provides search on the default branch (" +
						br +
						")."
				);
				return;
			...
			// q=foo+repo:exlcode/exlcode_test
			let q: string =
				query.contentPattern.pattern +
				"+repo:" +
				this.exlcodeService.repoName;
			let s: ExlcodeApiSearch = new exlcode.Search({
				query: encodeURIComponent(q)
			});
			s.code(null, (err: ExlcodeError, result: SearchResult) => {
				if (err) {
					if (err.error) {
						e(err.error);
					} else {
				...
				// Exlcode only provides search on forks if the fork has
				// more star ratings than the parent.
				if (result.items.length == 0 && this.exlcodeService.isFork()) {
					e(
						"Exlcode doesn't provide search on forked repos unless the star rating is greater than the parent repo."
					);
					return;
				}

				// Search on IModel's to get accurate search results. Exlcode's search results
				// are not complete and don't have line numbers.
				this.modelSearch(
					query.contentPattern.pattern,
					result.items.map(item => uri.file(item.path))
				).then(
					matches => {
						c({
							limitHit: result.incomplete_results,
							results: matches,
							stats: {
								fromCache: false,
								resultCount: matches.length,
								unsortedResultTime:
									Date.now() - fileWalkStartTime
							}
						});
					},
					() => {
						e("Exlcode error performing search.");
					}
				);
			});
		});
	...
	private modelSearch(pattern: string, uris: uri[]): TPromise<FileMatch[]> {
		...
		return new TPromise<FileMatch[]>((c, e) => {
			...
			let promises = uris.map(uri =>
				limiter.queue(() =>
					this.editorService.resolveEditorModel({ resource: uri })
				)
			);
			TPromise.join(promises).then(
				(models: ITextEditorModel[]) => {
					let matches: FileMatch[] = [];
					models.forEach(model => {
						var textEditorModel = <IModel>model.textEditorModel;
						let m = new FileMatch(textEditorModel.uri);
						textEditorModel
							.findMatches(pattern, false, false, false, true)
							.forEach(r => {
								let frag = textEditorModel.getLineContent(
									r.startLineNumber
								);
								m.lineMatches.push(
									new LineMatch(frag, r.startLineNumber, [
										[
											r.startColumn - 1,
	...
	private fileSearch(
		...
		let fileWalkStartTime = Date.now();
		let engine = new ExlcodeFileSearchEngine(
			config,
			this.exlcodeService.getCache()
		);
	...
	private fileMatchFromRawMatch(match: IRawFileMatch): FileMatch {
		let path = match.base
			? paths.join(match.base, match.relativePath)
			: match.relativePath;
