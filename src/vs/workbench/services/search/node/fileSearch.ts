...
// DESKTOP: import * as childProcess from 'child_process';
// DESKTOP: import {StringDecoder} from 'string_decoder';
import errors = require('vs/base/common/errors');
// import fs = require('fs');
import paths = require('vs/base/common/paths');
// DESKTOP: import {Readable} from "stream";
...
export class FileWalker {
	...
	constructor(config: IRawSearch, private cache: IExlcodeTreeCache) {
		this.config = config;
...
//		extfs.readdir(rootFolder, (error: Error, files: string[]) => {
		this.cache.readdir(rootFolder, (error: Error, files: string[]) => {
	...
	private checkFilePatternRelativeMatch(basePath: string, clb: (matchPath: string, size?: number) => void): void {
		...
		return this.cache.stat(absolutePath, (error, stat) => {
...
//			fs.lstat(currentAbsolutePath, (error, lstat) => {
			this.cache.lstat(currentAbsolutePath, (error, lstat) => {
				if (error || this.isCanceled || this.isLimitHit) {
					return clb(null);
				}
	...
	private statLinkIfNeeded(path: string, lstat: IExlcodeTreeStat, clb: (error: Error, stat: IExlcodeTreeStat) => void): void {
	...
	private realPathIfNeeded(path: string, lstat: IExlcodeTreeStat, clb: (error: Error, realpath?: string) => void): void {
		if (lstat.isSymbolicLink()) {
