...
import { Exlcode, Repository, User, Gist, Error as ExlcodeError } from "exlcode";
import { IExlcodeService } from "exlcode/exlcodeService";
import { IWorkspaceContextService } from "vs/platform/workspace/common/workspace";
import { IExlcodeTreeCache, IExlcodeTreeStat } from "exlcode/exlcodeTreeCache";
...
export class FileService implements IFileService {
	...
	private ref: string;
	private cache: IExlcodeTreeCache;
	...
	constructor(
		...
		*/
		this.repo = this.exlcodeService.exlcode.getRepo(
			this.exlcodeService.repoName
		);
		this.ref = this.exlcodeService.ref;
		this.cache = this.exlcodeService.getCache();
	}
	...
	public resolveContent(
		...
		return this.doResolveContent(resource, options, (resource, etag, enc) =>
			this.resolveFileContent(resource, etag, enc)
		);
	}
	...
	public resolveStreamContent(
		...
		return this.doResolveContent(resource, options, (resource, etag, enc) =>
			this.resolveFileStreamContent(resource, etag, enc)
		);
	}

	private doResolveContent<T extends IBaseStat>(
		resource: uri,
		options: IResolveContentOptions,
		contentResolver: (
			resource: uri,
	...
	public resolveContents(resources: uri[]): TPromise<IContent[]> {
		...
		resources.forEach(resource => {
			contentPromises.push(
				limiter.queue(() =>
					this.resolveFileContent(resource).then(
						content => content,
						error =>
							TPromise.as(null /* ignore errors gracefully */)
					)
				)
			);
		});
	...
	private updateGist(
		...
		return new TPromise<boolean>((c, e) => {
			...
			if (info.gist) {
				...
				let gist: Gist = new exlcode.Gist({ id: info.gist.id });
				gist.update(data, (err: ExlcodeError) => {
					if (err) {
						e(err);
					} else {
						c(true);
				...
				// Create
				let gist: Gist = new exlcode.Gist({});
				gist.create(data, (err: ExlcodeError) => {
					if (err) {
						e(err);
					} else {
						c(true);
	...
	private checkNotify(absolutePath: string) {
		if (this.options.settingsNotificationPaths) {
			...
			for (
				let i = 0;
				i < this.options.settingsNotificationPaths.length;
				i++
			) {
				if (
					absolutePath === this.options.settingsNotificationPaths[i]
				) {
					notify = true;
					break;
				}
			...
			if (notify) {
				setTimeout(() => {
					this.eventEmitter.emit("settingsFileChanged");
				}, 0);
			}
		}
	}
	...
	private updateGistContent(
		...
		return new TPromise<IFileStat>((c, e) => {
			this.exlcodeService.findGist(resource).then(
				info => {
					// 1.) check file
					return this.checkFile(absolutePath, options).then(
						exists => {
							let encodingToWrite = this.getEncoding(
								resource,
								options.encoding
							);
							let addBomPromise: TPromise<boolean> = TPromise.as(
								false
							);
							...
							return addBomPromise.then(addBom => {
								...
								if (
									...
									writeFilePromise = this.updateGist(
										info,
										parts[2],
										parts[3],
										value
								...
								// Otherwise use encoding lib
									...
									throw new Error(
										"exlcodeFileService.updateContent with non-UTF8 encoding not implemented yet"
									);
									// TODO:
			...
			return addBomPromise.then(addBom => {
				...
				if (!addBom && encodingToWrite === encoding.UTF8) {
					...
					writeFilePromise = new TPromise<void>((c, e) => {
						...
						let commitMessage =
							this.options.commitMessage || "Update " + path;
						this.repo.write(
							this.ref,
							path,
							value,
	...
	public createFolder(resource: uri): TPromise<IFileStat> {
		...
		return this.createFile(
			uri.file(newPath),
			"Git requires at least 1 file to be present in a folder."
		).then(
			(stat: IFileStat) => {
	...
	public rename(resource: uri, newName: string): TPromise<IFileStat> {
		...
		if (oldPath[0] == "/") oldPath = oldPath.slice(1, oldPath.length);
		let newPath = paths.join(paths.dirname(oldPath), newName);

		return this.moveExlcodeFile(oldPath, newPath).then(
			() => {
	...
	private forceExplorerViewRefresh() {
		...
		// many places.
		let event = new Files.LocalFileChangeEvent(
			new FileStat(
	...
	private deleteExlcodeFile(sourcePath: string): TPromise<boolean> {
		return new TPromise<void>((c, e) => {
			let commitMessage =
				this.options.commitMessage || "Deleted " + sourcePath;
			this.repo.delete(
				this.ref,
				sourcePath,
				commitMessage,
	...
	private copyExlcodeFile(
		sourcePath: string,
		targetPath: string
	): TPromise<boolean> {
		return new TPromise<boolean>((c, e) => {
			return this.resolveFileContent(uri.file(sourcePath)).then(
				(content: IContent) => {
	...
	private moveExlcodeFile(
		...
		return this.existsFile(uri.file(targetPath)).then(exists => {
			...
			return this.copyExlcodeFile(sourcePath, targetPath).then(
				(success: boolean) => {
					if (success) {
						return this.deleteExlcodeFile(sourcePath);
					} else {
						return this.deleteExlcodeFile(targetPath);
					}
				}
			);
		});
	...
	private doMoveOrCopyFile(
		...
		/*
		return TPromise.wrapError(<IFileOperationResult>{
			message: 'exlcodeFileService.doMoveOrCopyFile not implemented (' + sourcePath + ' -> ' + targetPath + ')',
			fileOperationResult: FileOperationResult.FILE_NOT_FOUND
		});
*/
				...
				return TPromise.as(true)
					.then(() => {
						...
						if (isSameFile) {
							...
							return this.copyExlcodeFile(sourcePath, targetPath);
						} else {
							return this.moveExlcodeFile(sourcePath, targetPath);
						}
					})
					.then(() => exists);
			});
	...
	public importFile(source: uri, targetFolder: uri): TPromise<IImportResult> {
		return TPromise.wrapError(<IFileOperationResult>{
			message:
				"exlcodeFileService.importFile not implemented (" +
	...
	public del(resource: uri): TPromise<void> {
		...
		if (absPath[0] == "/") absPath = absPath.slice(1, absPath.length);
		return new TPromise<void>(c => {
			return this.deleteExlcodeFile(absPath).then(() => {
				c(<void>null);
			});
		});
	}
	...
	private resolve(
		...
		if (this.isGistPath(resource)) {
			...
			return this.toStatResolver(resource).then(model =>
	...
	private toStatResolver(resource: uri): TPromise<StatResolver> {
		...
		return new TPromise<StatResolver>((c, e) => {
			this.cache.stat(
				absolutePath,
				(error: Error, result: IExlcodeTreeStat) => {
					if (error) {
						e(error);
					} else {
						c(
	...
	private isGistPath(resource: uri): boolean {
		...
		return (
			this.options.gistRegEx &&
			this.options.gistRegEx.test(this.toAbsolutePath(resource))
		);
	}

	private resolveGistFile(
		resource: uri,
		options: IResolveFileOptions
	): TPromise<IFileStat> {
		return this.exlcodeService.resolveGistFile(resource);
	}
			...
			(model): TPromise<IContent> => {
				...
				return new TPromise<IContent>((c, e) => {
					if ((<any>model).submodule_git_url) {
						...
						xhr({ type: "GET", url: url }).then(
							(xhr: XMLHttpRequest) => {
								if (xhr.status == 200) {
									...
									console.log(
										"Http error: " +
											http.getErrorStatusDescription(
												xhr.status
											) +
											" url: " +
											url
						...
						// Regular repo file
						this.repo.getBlobRaw(
							model.etag,
							(err: ExlcodeError, content: string | boolean) => {
								if (!err) {
									// The GitHub API wrapper we uses returns the boolean true for content when there is none!!
									result.value =
										content == true ? "" : <string>content;
									c(result);
								} else {
									console.log(
										"repo.getBlob error using sha " +
											model.etag
									);
									e(FileOperationResult.FILE_NOT_FOUND);
								}
	...
	private resolveFileStreamContent(
		...
		return this.resolveFileContent(resource, etag, enc).then<
			IStreamContent
		>(content => {
	...
	*/
		...
		if (
			...
			for (let i = 0; i < this.options.encodingOverride.length; i++) {
				...
				if (
					resource
						.toString()
						.indexOf(override.resource.toString() + "/") === 0
				) {
	...
	public watchFileChanges(resource: uri): void {
		console.log(
			"exlcodeFileService.watchFileChanges not implemented (" +
				resource.toString(true) +
				")"
		);
		/* TODO:
	...
	public unwatchFileChanges(arg1: any): void {
		...
		console.log(
			"exlcodeFileService.unwatchFileChanges not implemented (" +
				resource +
				")"
		);
...
export class StatResolver {
	...
	constructor(
		...
		this.mime = !this.isDirectory
			? baseMime.guessMimeTypes(resource.fsPath).join(", ")
			: void 0;
	...
	private addExlcodeFields(fileStat: IFileStat, exlcodeStat: IExlcodeTreeStat) {
		if (exlcodeStat.isSymbolicLink()) {
			(<any>fileStat).type = "symlink";
		}
		if (exlcodeStat.submodule_git_url) {
			(<any>fileStat).type = "submodule";
			(<any>fileStat).submodule_git_url = exlcodeStat.submodule_git_url;
		}
	}
	...
	public resolve(options: IResolveFileOptions): TPromise<IFileStat> {
		...
		// Add exlcode fields
		this.addExlcodeFields(fileStat, this.stat);
	...
	private resolveChildren(
		...
		this.cache.readdir(absolutePath, (error: Error, files: string[]) => {
			...
			flow.parallel(
				...
				(
					file: string,
					clb: (error: Error, children: IFileStat) => void
				) => {
					//let fileResource = uri.file(paths.resolve(absolutePath, file));
					let fileResource = uri.file(
						paths.makePosixAbsolute(paths.join(absolutePath, file))
					);
					// let fileStat: fs.Stats;
					let fileStat: IExlcodeTreeStat;
					let $this = this;
					...
					flow.sequence(
						...
						function countChildren(fsstat: IExlcodeTreeStat): void {
							fileStat = fsstat;
						...
						function resolve(childCount: number): void {
							let childStat: IFileStat = {
								...
								mime: !fileStat.isDirectory()
									? baseMime
											.guessMimeTypes(fileResource.fsPath)
											.join(", ")
									: void 0
							};

							// Add exlcode fields
							$this.addExlcodeFields(childStat, fileStat);
