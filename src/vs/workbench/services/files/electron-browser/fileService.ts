...
import {
	FileService as exlcodeFileService,
	IFileServiceOptions,
	IEncodingOverride
...
export class FileService implements IFileService {
	...
	constructor(
		...
		this.raw = new exlcodeFileService(
	...
	private registerListeners(): void {
		// Config Changes
		this.configurationChangeListenerUnbind = this.configurationService.onDidUpdateConfiguration(
	...
	public resolveContent(
		...
		let timerEvent = timer.start(
			timer.Topic.WORKBENCH,
			strings.format("Load {0}", contentId)
		);

		return this.raw.resolveContent(resource, options).then(
			result => {
				timerEvent.stop();
	...
	public resolveStreamContent(
		...
		return this.raw.resolveStreamContent(resource, options).then(
			result => {
				timerEvent.stop();
	...
	public updateContent(
		...
		let timerEvent = timer.start(
			timer.Topic.WORKBENCH,
			strings.format("Save {0}", resource.toString())
		);

		return this.raw.updateContent(resource, value, options).then(
			result => {
				timerEvent.stop();
