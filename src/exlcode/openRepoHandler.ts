...
class RepoQuickOpenEntry extends QuickOpenEntryGroup {
	...
	private openRepo(): void {
		if (this.info.full_name !== this.exlcodeService.repoName) {
			(<any>window).sendGa("/workbench/repo/open", () => {
				openRepository(this.info.full_name, <IWindowConfiguration>(
	...
	public run(mode: Mode, context: IContext): boolean {
		...
		// Ask exlcode if it is valid before loading. If not, show a message.
		...
		repo.show((err: Error, info: RepositoryInfo) => {
			if (!err) {
				...
				this.messageService.show(
					Severity.Error,
					"Error opening repository " + this.info.full_name
				);
			}
		});
...
export class OpenRepoHandler extends QuickOpenHandler {
	...
	private getInfoFromRepoInfo(info: RepositoryInfo): Info {
		let group: Group;
		if (
			info.owner.type === "User" &&
			info.owner.login ===
				this.exlcodeService.getAuthenticatedUserInfo().login
		) {
			group = Group.User;
	...
	private getRecentInfos(infos: Info[]): TPromise<Info[]> {
		...
		this.exlcodeService.getRecentRepos().forEach(name => {
			...
			if (index >= 0) {
				let copy = <Info>{};
				Object.keys(infos[index]).forEach(
					key => (copy[key] = infos[index][key])
				);
				...
				limiter.queue(
					() =>
						new TPromise<void>((c, e) => {
							...
							repo.show((err: Error, info: RepositoryInfo) => {
								if (!err) {
									infosRecent[indexPushed].description =
										info.description || "";
	...
	private getModel(): TPromise<QuickOpenModel> {
		return new TPromise<QuickOpenModel>((c, e) => {
			...
			this.exlcodeService.exlcode
				.getUser()
				.repos(
					{ per_page: 1000 },
					(err: Error, repositoryInfos: RepositoryInfo[]) => {
						if (err) {
							e(err);
							return;
						}
						...
						let infos = repositoryInfos.map(info =>
							this.getInfoFromRepoInfo(info)
						);
						this.getRecentInfos(infos).then(infosRecent => {
							...
							let infosSorted = infos.sort((a, b) => {
								...
								n = a.login
									.toLowerCase()
									.localeCompare(b.login.toLowerCase());
								if (n != 0) return n;
								return a.name
									.toLowerCase()
		...
		return new TPromise<QuickOpenModel>((c, e) => {
			this.getModel().then(
				(model: QuickOpenModel) => {
					...
					let entries: RepoQuickOpenEntry[] = <RepoQuickOpenEntry[]>(
						model.getEntries()
					);
					...
					entries.forEach((e, index) => {
						...
						if (index === 0) {
							if (
								!searchValue ||
								entries
									.map(entry => entry.info.full_name)
						...
						if (searchValue) {
							...
							} = QuickOpenEntry.highlight(e, searchValue, true);
							e.setHighlights(
								labelHighlights,
								descriptionHighlights
							);
						...
						if (
							indexLastVisible < 0 ||
							entries[indexLastVisible].info.group != e.info.group
						) {
