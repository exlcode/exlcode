 ...
 *            Exlcode.js is freely distributable.
 *
...
(function (root, factory) {
	console.log("Loading exlcode.js");
	/* istanbul ignore next */
	if (typeof define === 'function' && define.amd) {
		define(
			[
				'exlcode/lib/es6-promise',
				'exlcode/lib/base-64',
				'exlcode/lib/utf8',
				'exlcode/lib/axios'
			],
			function (Promise, Base64, Utf8, axios) {
				return (root.Exlcode = factory(Promise, Base64, Utf8, axios));
			}
		);
	} else if (typeof module === 'object' && module.exports) {
		module.exports = factory(require('exlcode/lib/es6-promise'), require('exlcode/lib/base-64'), require('exlcode/lib/utf8'), require('exlcode/lib/axios'));
	} else {
		root.Exlcode = factory(root.Promise, root.base64, root.utf8, root.axios);
	}
	...
	var Exlcode = function (options) {
		...
		var _request = Exlcode._request = function _request(method, path, data, cb, accept, raw_response) {
			function getURL() {
		...
		var _requestAllPages = Exlcode._requestAllPages = function _requestAllPages(path, cb) {
			var results = [];
		...
		Exlcode.User = function () {
			...
			this.userStarred = function (username, cb) {
				// Exlcode does not always honor the 1000 limit so we want to iterate over the data set.
		...
		Exlcode.Repository = function (options) {
			...
			this.getTreeRecursive = function (sha, cb) {
				_request('GET', repoPath + '/git/trees/' + sha, {
					recursive: 1
		...
		Exlcode.Issue = function (options) {
		...
		Exlcode.Search = function (options) {
			var path = '/search/';
	...
	Exlcode.getIssues = function (user, repo) {
		return new Exlcode.Issue({
			user: user,
			repo: repo
		});
	};

	Exlcode.getRepo = function (user, repo) {
		if (!repo) {
			return new Exlcode.Repository({
				fullname: user
			});
		} else {
			return new Exlcode.Repository({
	...
	Exlcode.getUser = function () {
		return new Exlcode.User();
	};

	Exlcode.getOrg = function () {
		return new Exlcode.Organization();
	};

	Exlcode.getGist = function (id) {
		return new Exlcode.Gist({
			id: id
		});
	};

	Exlcode.getSearch = function (query) {
		return new Exlcode.Search({
			query: query
		});
	};

	Exlcode.getRateLimit = function () {
		return new Exlcode.RateLimit();
	};
